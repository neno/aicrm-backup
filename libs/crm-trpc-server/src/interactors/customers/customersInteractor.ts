...
import { db, DB } from '../../db/db-client';
...
export default function makeCustomersInteractor(db: DB) {
  const all = GetAllCustomers.implement(async ({ limit, offset, orderBy }) => {
  ...
  const search = SearchCustomers.implement(
  ...
  const create = CreateCustomer.implement(async (input) => {
    const res = await db
      .insert(customers)
      .values(input)
  ...
  const findById = FindCustomerById.implement(async (input) => {
...
export type CustomersInteractor = typeof customersInteractor;
